...
const SERIES_QUERY = `
  query {
    Page(page: 1, perPage: 20) {
      media(type: ANIME, format: TV, sort: SCORE_DESC, isAdult: false) {
        id
...
const ErrorState = ({ message, onRetry }) => (
  <div className="flex flex-col items-center justify-center gap-4 px-5 py-14 w-full">
...
const AnimeSeries = () => {
  ...
  const fetchSeries = useCallback(async () => {
    ...
    try {
      ...
      setError("Failed to load anime series.");
      console.error(err);
    } finally {
      setLoading(false);
    }
