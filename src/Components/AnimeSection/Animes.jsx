...
// Returns a human-readable episode/format label for a given anime entry
const getEpisodeLabel = (format, episodes) => {
  if (format === "MOVIE") return "Movie";
  if (episodes) return `${episodes} Episodes`;
  return "Ongoing";
};
...
const AllAnimes = () => {
  ...
  const fetchAnimes = useCallback(async (pageNum) => {
    try {
      ...
      const formatted = media.map((anime) => ({
        ...
        episodes: getEpisodeLabel(anime.format, anime.episodes),
  ...
  if (initialLoading) {
    return (
      <div className="px-5 pt-8 pb-12">
        <h1 className="text-3xl font-bold text-white mb-8">All Animes</h1>
