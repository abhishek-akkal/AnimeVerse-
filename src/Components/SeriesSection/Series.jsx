...
const Series = () => {
  ...
  const fetchSeries = useCallback(async (pageNum) => {
    try {
  ...
  const rows = chunkArray(seriesData, CARDS_PER_ROW);
  ...
  return (
    <div className="px-5 pt-8 pb-12">
      ...
      <div className="hidden lg:grid grid-cols-5 gap-6">
        {seriesData.map((anime) => (
          <SeriesCard key={anime.id} anime={anime} />
        ))}
      </div>
